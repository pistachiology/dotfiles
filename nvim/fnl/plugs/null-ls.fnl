...
(fn setup []
  ...
  (local eslint (helper.conditional (fn [utils] 
    (let [project-local-bin "node_modules/.bin/eslint"
          ...
          cfg {:command (or (and (utils.root_has_file project-local-bin) project-local-bin) "eslint" )}]
      (eslint cfg)))))

  (local eslint-format (helper.conditional (fn [utils] 
    (let [project-local-bin "node_modules/.bin/eslint"
  ...
  (local prettier (helper.conditional (fn [utils] 
    (let [project-local-bin "node_modules/.bin/prettier"
          ...
          cfg {:command (or (and (utils.root_has_file project-local-bin) project-local-bin) "prettier" )}]
      (prettier cfg)))))
